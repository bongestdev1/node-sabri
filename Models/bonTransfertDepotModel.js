...
const schemaBonTransfertDepot=mongoose.Schema({ 
...
schemaBonTransfertDepot.plugin(mongoosePaginate);

schemaBonTransfertDepot.method("toJSON", function() {
    const { __v, _id, ...object } = this.toObject();
    object.id = _id;
    return object;
});

const BonTransfertDepot = mongoose.model('BonTransfertDepot',schemaBonTransfertDepot)

async function getNumeroAutomatique(idSociete, exercice){
    var exerciceString = exercice+""
    let lastDoc = (await BonTransfertDepot.find({societe:idSociete, exercice:exerciceString}).sort({_id: -1}).limit(1))[0];
...
module.exports.BonTransfertDepot=BonTransfertDepot
