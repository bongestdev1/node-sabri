...
router.post('/newRole', async(req,res)=>{
    ...
    console.log("societeRacine = ", body)
 ...
 */
 router.get('/getAllParametres/:id', async (req, res) => {
    ...
    return res.send({ status: true, roles: roles })
 ...
 *
 */
router.post('/modifierRole/:id', async(req,res)=>{
   ...
   // const {error}=validateRole(req.body)
    ...
    if(!role) return res.status(401).send({status:false})


    const result = await Role.findOneAndUpdate({_id:req.params.id}, req.body)

    const role2 = await Role.findById(req.params.id)

    return res.send({status:true,resultat:role2})
 ...
 *         description: Some error happened
 */
router.post('/deleteRole/:id',  async(req,res)=>{
try{
    //if(req.user.user.role != "admin") return res.status(401).send({status:false})
    ...
    if(await Role.findOneAndDelete({_id:req.params.id})){
 ...
 *
 */
 router.post('/listRoles', async(req,res)=>{
    ...
    if(listFilter.length > 1){
      result = await  Role.paginate({$and:listFilter}, options) 
    }else if(listFilter.length == 1){
      result = await  Role.paginate(listFilter[0], options)
    }else{
      result = await  Role.paginate({}, options)
    }
...
router.get('/getById/:id', verifytoken, async(req,res)=>{
    ...
    return res.send({status:true,resultat:role})
...
module.exports.routerRole=router
